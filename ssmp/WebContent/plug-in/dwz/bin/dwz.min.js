...
function initUI($p){
	...
	$("a[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			...
			navTab.openTab(tabid, url,{title:title, fresh:fresh, external:external,loaded:loaded});
	...
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			//自定义添加,加载成功后回调函数
			options.loaded = $this.attr('loaded');
				...
				if (op.external || url.isExternalUrl()) {
					...
					$panel.ajaxUrl({
						type:op.type, url:url, data:op.data, callback:function(){
							navTab._loadUrlCallback($panel);
							if(typeof op.loaded == 'string'){
		                        op.loaded = lu[op.loaded];
		                    }
		                    if($.isFunction(op.loaded)) op.loaded(op,$panel);
						}
					});
				}
			}
			...
			if(dialog) {
				...
				if(op.fresh || url != $(dialog).data("url")){
					...
					jDContent.loadUrl(url, {}, function(){
	                    ...
	                    if($.isFunction(op.loaded)) op.loaded(op,dialog);
					});
				}
